...
void limpar_o_display(uint8_t *ssd, struct render_area *frame_area);
void escrever_display(uint8_t *ssd, struct render_area *frame_area, char *text[], uint size);
...
void escrever_display(uint8_t *ssd, struct render_area *frame_area, char *text[], uint size) {
    ...
    render_on_display(ssd, frame_area); // Sem "&", pois já é ponteiro
}
